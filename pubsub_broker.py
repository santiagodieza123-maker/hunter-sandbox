...
class PubSubBroker:
    ...
    async def _send_safe(self, topic: str, client: Client, message: str):
        try:
            ...
            # Cliente zombie: descarte atomico del set ORIGINAL (no del snapshot).
            if topic in self.topics:
                self.topics[topic].discard(client)
        except asyncio.TimeoutError:
            # Suscriptor lento: se descarta este envio puntual sin bloquear a los demas.
            pass

    async def publish(self, topic: str, message: str):
        if topic not in self.topics:
            return
        # Snapshot inmutable: evita RuntimeError si un callback muta
        # self.topics[topic] durante la iteracion.
        snapshot = list(self.topics[topic])
        # Despacho concurrente: evita backpressure por suscriptores lentos.
